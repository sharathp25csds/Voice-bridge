...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (navHomeBtn) {
        navHomeBtn.addEventListener('click', (e) => {
            ...
            if (cm) { cm.setAttribute('inert', ''); cm.style.display = 'none'; }
            const am = document.getElementById('authModal');
            if (am) { am.setAttribute('inert', ''); am.style.display = 'none'; }
            window.location.hash = '#Home';
        });
    }
    ...
    const langMap = {
        ...
        'ta-IN': 'ta-IN', 'te-IN': 'te-IN', 'mr-IN': 'mr-IN', 'ml-IN': 'ml-IN'
    };
...
if (speakBtn) {
    ...
    const initPeer = () => {
        ...
        peer.on('disconnected', () => {
            ...
            setStatus('Connection lost. Reconnecting...', 'ready');
            peer.reconnect();
        });
        ...
        peer.on('error', (err) => {
            ...
            if (err.type === 'peer-unavailable') {
                setStatus('Peer not found. Please check the ID.', 'error');
            } else if (err.type === 'network') {
                setStatus('Network error. Check your internet.', 'error');
            } else if (err.type === 'server-error' || err.type === 'socket-error') {
                setStatus('Server error. Retrying in 3s...', 'error');
                setTimeout(() => { if (!peer || peer.destroyed) initPeer(); }, 3000);
            } else {
